...
void ipc_init(struct sway_server *server) {
	...
	setenv("TILEWINSOCK", ipc_sockaddr->sun_path, 1);
...
struct sockaddr_un *ipc_user_sockaddr(void) {
	...
	if (path_size <= snprintf(ipc_sockaddr->sun_path, path_size,
			"%s/tilewin-ipc.%u.%i.sock", dir, getuid(), getpid())) {
...
void ipc_event_tilewin(const char *change, json_object *data) {
	if (!ipc_has_event_listeners(IPC_EVENT_TILEWIN)) {
		json_object_put(data);
		return;
	}
	json_object *obj = data ? data : json_object_new_object();
	json_object_object_add(obj, "change", json_object_new_string(change));
	ipc_send_event(json_object_to_json_string(obj), IPC_EVENT_TILEWIN);
	json_object_put(obj);
}
...
void ipc_client_handle_command(struct ipc_client *client, uint32_t payload_length,
	...
	case IPC_SUBSCRIBE:
	{
		...
		for (size_t i = 0; i < json_object_array_length(request); i++) {
			...
			} else if (strcmp(event_type, "tilewin") == 0) {
				client->subscribed_events |= event_mask(IPC_EVENT_TILEWIN);
			} else {
				const char msg[] = "{\"success\": false}";
				ipc_send_reply(client, payload_type, msg, strlen(msg));
	...
	case IPC_GET_TILEWIN:
	{
		json_object *state = tw_describe_state();
		const char *json_string = json_object_to_json_string(state);
