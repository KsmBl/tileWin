...
static const char usage[] =
	...
	"  -m, --mode <mode>      Start in 'tile' or 'window' mode.\n"
	"\n";
...
int main(int argc, char **argv) {
	...
	char *mode_override = NULL;

	int c;
	while (1) {
		int option_index = 0;
		c = getopt_long(argc, argv, "hCdD:vVc:m:", long_options, &option_index);
		if (c == -1) {
			break;
		}
		switch (c) {
		...
		case 'u':
			...
			break;
		case 'v': // version
			printf("tilewin version " SWAY_VERSION "\n");
			exit(EXIT_SUCCESS);
			break;
		case 'V': // verbose
			verbose = true;
