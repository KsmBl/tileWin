...
static void arrange_container(struct sway_container *con,
	...
	if (con->view && con->current.tw_deco) {
		...
		wlr_scene_rect_set_size(con->border.left, 0, 0);
		wlr_scene_rect_set_size(con->border.right, 0, 0);
		tw_deco_arrange(con, width, height);
		wlr_scene_node_reparent(&con->view->scene_tree->node, con->content_tree);
		wlr_scene_node_set_position(&con->view->scene_tree->node, in.left, in.top);
		wlr_scene_node_set_position(&con->view->output_handler->node, -in.left, -in.top);
...
static void arrange_workspace_floating(struct sway_workspace *ws) {
	for (int i = 0; i < ws->current.floating->length; i++) {
		...
		if (floater->current.tw_minimized) {
