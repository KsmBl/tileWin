...
struct sway_container *container_create(struct sway_view *view) {
	...
	if (view) {
		c->tw.deco_tree = alloc_scene_tree(c->scene_tree, &failed);
		if (c->tw.deco_tree) {
			wlr_scene_node_set_enabled(&c->tw.deco_tree->node, false);
		}
	}
...
void container_set_geometry_from_content(struct sway_container *con) {
	...
	if (con->pending.tw_deco) {
		...
		con->pending.x = con->pending.content_x - in.left;
		con->pending.y = con->pending.content_y - in.top;
		con->pending.width = con->pending.content_width + in.left + in.right;
		con->pending.height = con->pending.content_height + in.top + in.bottom;
		node_set_dirty(&con->node);
		return;
	}
