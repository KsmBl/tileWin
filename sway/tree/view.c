...
void view_autoconfigure(struct sway_view *view) {
	...
	if (con->pending.tw_deco) {
		struct tw_insets in = tw_deco_insets(con->pending.tw_maximized);
		con->pending.content_x = con->pending.x + in.left;
		con->pending.content_y = con->pending.y + in.top;
		con->pending.content_width = fmax(con->pending.width - in.left - in.right, 1);
		con->pending.content_height = fmax(con->pending.height - in.top - in.bottom, 1);
		return;
	}
...
static void handle_foreign_minimize_request(
	...
	if (view->container) {
		tw_minimize(view->container, event->minimized);
		transaction_commit_dirty();
	}
}
...
static void handle_foreign_maximize_request(
	...
	if (view->container && container_is_floating(view->container)) {
		tw_maximize(view->container, event->maximized);
		transaction_commit_dirty();
	}
}
...
void view_map(struct sway_view *view, struct wlr_surface *wlr_surface,
	...
	if (tw_mode == TW_MODE_WINDOW && !fullscreen) {
		if (!container_is_floating(view->container)) {
			view->container->tw.auto_floated = true;
			container_set_floating(view->container, true);
			tw_place_new_window(view->container);
		} else {
...
bool view_is_visible(struct sway_view *view) {
	...
	if (view->container->pending.tw_minimized) {
		return false;
	}
