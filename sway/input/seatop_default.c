...
static enum wlr_edges find_edge(struct sway_container *cont,
	...
	if (cont->current.tw_deco) {
		enum wlr_edges edges = WLR_EDGE_NONE;
		if (tw_deco_hit_test(cont, cursor->cursor->x, cursor->cursor->y,
				&edges) != TW_HIT_EDGE) {
			return WLR_EDGE_NONE;
		}
		return edges;
...
static void handle_button(struct sway_seat *seat, uint32_t time_msec,
	...
	if (tw_handle_button(seat, time_msec, cont, surface, button,
...
static void handle_pointer_motion(struct sway_seat *seat, uint32_t time_msec) {
	...
	struct sway_container *hovered = node && node->type == N_CONTAINER ?
		node->sway_container : NULL;
	bool tw_cursor = tw_handle_motion(seat, hovered, surface);
