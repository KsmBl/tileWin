...
static void config_defaults(struct sway_config *config) {
	if (!(config->swaynag_command = strdup("tilewin-nag"))) goto cleanup;
...
static char *get_config_path(void) {
	const char *file = tw_mode == TW_MODE_WINDOW ?
		"windowmode.conf" : "tilemode.conf";
	char *config_dir = tw_config_dir();
	char *candidates[] = {
		config_dir ? format_str("%s/%s", config_dir, file) : NULL,
		format_str("%s/tileWin/%s", SYSCONFDIR, file),
		format_str("%s/config/%s", tw_data_dir(), file),
	};
	free(config_dir);

	char *path = NULL;
	for (size_t i = 0; i < sizeof(candidates) / sizeof(candidates[0]); i++) {
		if (!path && candidates[i] && file_exists(candidates[i])) {
			path = candidates[i];
		} else {
			free(candidates[i]);
		}
