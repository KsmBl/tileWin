...
| Menus | Right-click menus of the taskbar, taskbar buttons and start button (with submenus), the style of the start menu, pinned apps, places and power entries of the start menu. **Choose...** on every entry picks an installed app, one of tileWin's own actions (arrange windows, show the desktop, task view, run, lock, shut down, switch theme, ...) or a folder and fills in the label, icon and command; the icon button opens an icon picker and a second button makes an entry bold, checked or greyed out |
