...
int main(int argc, char **argv) {
	...
	if (strcasecmp(cmdtype, "command") == 0) {
		type = IPC_COMMAND;
	} else if (strcasecmp(cmdtype, "get_tilewin") == 0) {
		type = IPC_GET_TILEWIN;
		raw = true;
	...
	// "mode tile|window|toggle" switches the tileWin mode; other "mode"
	// arguments keep selecting sway binding modes.
	if (type == IPC_COMMAND && (strcmp(command, "mode tile") == 0 ||
			strcmp(command, "mode window") == 0 || strcmp(command, "mode toggle") == 0)) {
		char *rewritten = format_str("wm_%s", command);
		free(command);
		command = rewritten;
	}
