...
static const struct tw_widget_option opts_clock[] = {
	{ "format", "Format", "strftime format, \\n starts a second line. The theme decides by default.",
		NULL },
	{ "tooltip_format", "Tooltip format", "strftime format of the tooltip", NULL },
	{ "settings", "Settings link", "Opened by the link in the calendar, default exec tilewin-settings --page datetime",
		NULL },
	{ 0 },
};
...
static const struct tw_widget_option opts_gpu[] = {
	...
	{ "command", "Command", "For cards that report nothing in /sys, e.g. nvidia-smi --query-gpu=utilization.gpu --format=csv,noheader,nounits",
		NULL },
	{ "style", "Style", "Text, a chart of the last measurements or a bar", choice_meter },
	{ "width", "Width", "Pixels, for the chart and the bar", NULL },
	{ "warning", "Warning above", "Percent; the text turns to the warning color", NULL },
...
static const struct tw_widget_option opts_net[] = {
	...
	{ "max_rate", "Full scale", "KiB per second the chart and the bar are drawn against, default 12500",
		NULL },
	{ "style", "Style", "Text, a chart of the last measurements or a bar", choice_meter },
	{ "width", "Width", "Pixels, for the chart and the bar", NULL },
	{ "warning", "Warning above", "Percent; the text turns to the warning color", NULL },
...
static const struct tw_widget_option opts_power[] = {
	...
	{ "max_watts", "Full scale", "Watts the chart and the bar are drawn against, default 60",
		NULL },
	{ "style", "Style", "Text, a chart of the last measurements or a bar", choice_meter },
	{ "width", "Width", "Pixels, for the chart and the bar", NULL },
	{ "warning", "Warning above", "Percent; the text turns to the warning color", NULL },
...
static const struct tw_widget_option opts_disk[] = {
	...
	{ "format", "Format", "{rate} is e.g. 1.2 MB/s, {kbps} the plain number; empty shows only the lamp",
		NULL },
	{ "fg", "Color", "e.g. #7eb8f7; the theme decides by default", NULL },
	{ 0 },
};
static const struct tw_widget_option opts_battery[] = {
	...
	{ "format_charging", "Format while charging", "Used instead of Format while the battery charges",
		NULL },
	{ "format_full", "Format when full", "Used instead of Format once the battery is full", NULL },
	{ "format_plugged", "Format on mains", "Used instead of Format while the charger is plugged in",
		NULL },
	...
	{ "quick_settings", "Click opens quick settings", "The Windows 11 flyout instead of the battery flyout",
		choice_yes_no },
	{ "settings", "Settings link", "Opened by the link in the flyout, e.g. exec tilewin-settings --page screen",
		NULL },
	{ 0 },
};
static const struct tw_widget_option opts_network[] = {
	...
	{ "format_ethernet", "Format on a cable", "Used instead of Format for a wired connection",
		NULL },
	{ "format_disconnected", "Format when offline", "Used instead of Format while nothing is connected",
		NULL },
...
static const struct tw_widget_option opts_volume[] = {
	...
	{ "quick_settings", "Click opens quick settings", "The Windows 11 flyout instead of the volume flyout",
		choice_yes_no },
	{ 0 },
};
...
static const struct tw_widget_option opts_notifications[] = {
	{ "always", "Always show the button", "Otherwise it appears only when something is waiting",
		choice_yes_no },
	{ 0 },
};
static const struct tw_widget_option opts_taskbar[] = {
	{ "icons_only", "Icons only", "\"theme\" follows the theme (Windows 7 and 11 show icons only)",
		choice_theme_yes_no },
	{ "group", "Combine windows of the same app", NULL, choice_yes_no },
	{ "workspaces", "Show windows of", "The current workspace or all workspaces",
		choice_current_all },
...
static const struct tw_widget_option opts_custom[] = {
	...
	{ "exec_listen", "Streaming command", "Instead of Command: keeps running and prints one line (or JSON) per update",
		NULL },
...
/*
 * The looks on the desktop. "compact" is the widget as the taskbar draws it,
 * only bigger; the others are drawn for the desktop alone, in the colors of
 * the theme and its light or dark scheme.
 */
static const struct tw_widget_style styles_plain[] = {
	{ "compact", "Like on the taskbar", 0, 1 },
	{ "tile", "Tile with its name", 2, 2 },
	{ 0 },
};
static const struct tw_widget_style styles_meter[] = {
	{ "compact", "Like on the taskbar", 0, 1 },
	{ "chart", "Chart of the last measurements", 3, 2 },
	{ "gauge", "Gauge with a needle", 2, 2 },
	{ "ring", "Ring", 2, 2 },
	{ "bar", "Bar", 3, 1 },
	{ "tile", "Tile with its name", 2, 2 },
	{ 0 },
};
static const struct tw_widget_style styles_level[] = {
	{ "compact", "Like on the taskbar", 0, 1 },
	{ "ring", "Ring", 2, 2 },
	{ "gauge", "Gauge with a needle", 2, 2 },
	{ "bar", "Bar", 3, 1 },
...
static const struct tw_widget_style styles_disk[] = {
	{ "compact", "Like on the taskbar", 0, 1 },
	{ "chart", "Chart of the last measurements", 3, 2 },
	{ "tile", "Tile with its name", 2, 2 },
	{ 0 },
};
static const struct tw_widget_style styles_clock[] = {
	{ "compact", "Like on the taskbar", 0, 1 },
	{ "digital", "Big digits with the date", 3, 2 },
	{ "analog", "Analog clock", 2, 2 },
	{ "binary", "Binary clock", 3, 2 },
	{ "tile", "Tile with its name", 2, 2 },
	{ 0 },
};
...
const struct tw_widget_info tw_widgets[] = {
	{ "start", "Start button", "Opens the start menu", BOTH, opts_start, styles_plain },
	{ "search", "Search box", "Opens the start menu to search apps",
		BOTH, opts_search, styles_plain },
	...
	{ "keyboard", "Keyboard layout", "Current layout, click to pick another",
		STATUS, opts_none, styles_plain },
	{ "volume", "Volume", "Speaker volume", STATUS, opts_volume, styles_level },
	{ "network", "Network", "Connection status", STATUS, opts_network, styles_level },
	{ "battery", "Battery", "Charge level", STATUS, opts_battery, styles_level },
	{ "brightness", "Brightness", "Screen brightness", STATUS, opts_brightness, styles_level },
	{ "cpu", "CPU usage", "Processor load", STATUS, opts_cpu, styles_meter },
	{ "memory", "Memory usage", "RAM in use, click for a flyout",
		STATUS, opts_memory, styles_meter },
	{ "disk", "Disk activity", "A lamp that lights up while the disks are busy",
		STATUS, opts_disk, styles_disk },
	{ "gpu", "GPU usage", "Load of a graphics card", STATUS, opts_gpu, styles_meter },
	{ "net", "Network usage", "What goes through an interface", STATUS, opts_net, styles_meter },
	{ "storage", "Disk space", "How full a file system is", STATUS, opts_storage, styles_meter },
	{ "power", "Power draw", "Watts the computer is drawing", STATUS, opts_power, styles_meter },
...
const struct tw_widget_option tw_widget_desktop_options[] = {
	// the choices of "style" are the styles of the widget
	{ "style", "Style", "How the widget looks on the desktop", NULL },
	{ "column", "Column", "Cell of the grid from 0 at the left; negative counts from the right, "
		"-1 is the last column. Empty puts it at the right edge", NULL },
	{ "row", "Row", "Counted from 0 at the top; a negative number counts from the bottom", NULL },
	{ "size", "Size", "Cells it takes, columns x rows, e.g. 3x2; the style decides by default",
		NULL },
	{ "seconds", "Seconds", "The analog and the binary clock show the seconds", choice_yes_no },
...
const struct tw_widget_style *tw_widget_style_find(const struct tw_widget_info *info,
	...
	for (const struct tw_widget_style *style = info->styles; name && style->name; style++) {
